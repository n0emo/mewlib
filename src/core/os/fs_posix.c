...
bool mew_fs_get_size(const char *path, uintptr_t *size) {
    ...
    if (size != NULL) {
        *size = (uintptr_t)s.st_size;
    }
    return true;
}

bool mew_fs_read_file_to_sb(const char *path, StringBuilder *sb) {
    bool result = true;
    ...
    char buf[1024] = {0};
    while ((long)sb_count(sb) < stat_buf.st_size) {
        ptrdiff_t size = read(fd, buf, sizeof(buf));
        if (size < 0) {
            return_defer(false);
        }
        sb_append_buf(sb, buf, (size_t)size);
    }
